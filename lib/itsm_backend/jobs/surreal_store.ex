defmodule ItsmBackend.Jobs.SurrealStore do
  ...
  def claim_oldest(lease_seconds)
    ...
    params = %{
      "claimed_at" => encode_datetime(claimed_at),
      "lease_expires_at" => encode_datetime(lease_expires_at)
    }
    ...
    with {:ok, response} <-
      ...
      case result do
        ...
        other ->
          {:error,
           {
             :unexpected_expired_processing_result,
             other
           }}
      end
    end
  ...
  def find_oldest_expired_processing(now) do
    {:error,
     {
