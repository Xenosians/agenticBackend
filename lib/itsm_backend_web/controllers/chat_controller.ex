    ...
    case AuthRequest.require_authenticated(conn, csrf: true) do
      {:ok, conn, ctx} ->
        case Chats.create(ctx.user, params) do
          ...
          {:error, _} ->
            conn |> put_status(:internal_server_error) |> json(%{error: "chat_create_failed"})
        end
    ...
    case AuthRequest.require_authenticated(conn) do
      {:ok, conn, ctx} ->
        case Chats.list(ctx.user) do
          ...
          {:error, _} ->
            conn |> put_status(:internal_server_error) |> json(%{error: "chat_lookup_failed"})
        end
    ...
    case AuthRequest.require_authenticated(conn) do
      {:ok, conn, ctx} ->
        ...
        case Chats.history(ctx.user, chat_id, limit) do
          {:ok, messages} ->
            json(conn, %{chat_id: chat_id, messages: messages})
          ...
          {:error, _} ->
            conn |> put_status(:internal_server_error) |> json(%{error: "chat_history_failed"})
        end
